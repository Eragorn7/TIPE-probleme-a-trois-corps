...
def Calcul_pts_Lagrange():
    global TxL, TyL    
    #-- valeurs approchÃ©es pour 0,1,2
    TxL= [1-(nu/3)**(1/3.), 1+(nu/3)**(1/3.), -1-5*nu/13, 0.5-nu, 0.5-nu]
    TyL = [0,0,0, sqrt(3)/2., -sqrt(3)/2.]
    
    #-- valeurs precises trouvÃ©es par dichotomie
    x1,x2 = -1e3, -nu-1e-8 # encadrement de dÃ©part 
    TxL[2] = Dicho(x1,x2)
    
    x1,x2 = -nu+1e-8, 1-nu-1e-8 # encadrement de dÃ©part 
    TxL[0] = Dicho(x1,x2)
    
    x1,x2 = 1-nu+1e-8, 1e3 # encadrement de dÃ©part 
    TxL[1] = Dicho(x1,x2)
...
#--------- fonction pour section de PoincarÃ©
...
Utilise la fonction f(y) \in R qui definit la section de Poincare par f(y)=0 et passage de f>0 Ã  f<0
...
def Application_de_Poincare(y0,t):
    ...
    """ 
    Ty = [y0] #condition initiales encapsulÃ©e dans liste  
    Ly = np.array(y0) # memorise
    
    #-- on passe intervalle Dt
    Tt = [t, t+Dt]
    Ty = odeint(V, y0, Tt)
    y0 = Ty[len(Ty)-1] # coordonnÃ©es finales
    ...
    entree: Tt: intervalle en temps [t,t+Dt]
           Ty : intervalle en position [y(t), y(t+Dt)]
               tels que les points y(t) et y(t+Dt) sont de part et d'autre de la Section de PoincarÃ©
    """ 
    ...
    while((Tt[1]-Tt[0])>1e-10): # precision sur t
        ...
        if(fm>0): # on conserve la 2eme moitiÃ©
            Tt[0] = tm
            Ty[0] = Ty2[1]
        else: # on conserve la 1ere moitiÃ©
            Tt[1] = tm
            Ty[1] = Ty2[1]
...
#--definit l'application  de PoincarÃ© sur R2
"""
entree x,y
sortie x2,y2
"""
def fS(x,y):
    ...
    if(Ec>=0):
       ...
       print('Erreur: Ec<0 dans Section de PoincarÃ©, zone non accessible.')
       
    y0 = [x,px,y,py] # point de dÃ©part sur la section de PoincarÃ©
    t=0 # arbitraire
...
def fS2(x,px):
    ...
    if(res>=0):
       ...
       print('Erreur: res<0 dans Section de PoincarÃ©, zone non accessible.')
...
def Dessin_Section_xy():
    ...
    plt.title('Application PoincarÃ©. nu='+ ('%4.3f'%nu)+ ', E='+ ('%4.3f'%E))
...
Sortie: dessin de la trajectoire sur le plan(x,y), referentiel galilÃ©en
...
# Partie 5 ######################################
# ---- Section de PoincarÃ© 2D en (x,px)
Dt=1 # temps minimal entre deux sections
Dessin_Section_xpx()
